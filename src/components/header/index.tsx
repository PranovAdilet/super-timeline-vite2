...
import { ITimelineScaleState } from "@/shared/types";
import {
  ACTIVE_CLONE,
  ACTIVE_DELETE,
  ACTIVE_SPLIT,
...
const Header = () => {
  ...
  const doActiveSplit = () => {
    eventBus.dispatch(ACTIVE_SPLIT, {
      payload: {},
      options: {
        time: getCurrentTime(),
      },
  ...
  const changeScale = (scale: ITimelineScaleState) => {
    eventBus.dispatch(TIMELINE_SCALE_CHANGED, {
      payload: {
        scale,
      },
    });
  ...
  const handlePlay = () => {
    eventBus.dispatch(PLAYER_PLAY);
  };

  const handlePause = () => {
    eventBus.dispatch(PLAYER_PAUSE);
  };
        ...
        >
          <div className="flex px-2">
            ...
            <Button
              disabled={!activeIds.length}
              onClick={() => {
                eventBus.dispatch(ACTIVE_CLONE);
              }}
              variant={"ghost"}
              size={"sm"}
              className="flex items-center gap-1 px-2"
